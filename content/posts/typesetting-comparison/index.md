...
For example, you can type `<=` to get "$\leq$", `RR` to get "$\mathbb{R}$", `oo` to get "$\inf$", `->` to get "$\to$", `|->` to get "$\mapsto$", and, well, I guess I don't need to explain how to get a longer arrow.
